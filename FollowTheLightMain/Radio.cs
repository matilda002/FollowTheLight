...
public class Radio
{
    ...
    private const string DbUri = "Host=localhost;Port=5455;Username=postgres;Password=postgres;Database=followthelightdb;";
    ...
    public void GameMessage(HttpListenerResponse res)
    {
        ...
        Console.WriteLine("[ Player receiving message ]");
    ...
    {
        using (new NpgsqlConnection(DbUri))
        {
            try
            {
                StreamReader reader = new StreamReader(req.InputStream, req.ContentEncoding);
    ...
    private static void SendResponseOk(HttpListenerResponse res, string content)
    {
    ...
    private static void SendResponseEx(HttpListenerResponse res, string content)
    {
        byte[] buffer = Encoding.UTF8.GetBytes(content);
        res.ContentType = "text/plain";
